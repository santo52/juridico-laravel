...
class ActuacionCollection extends Collection
{
    public function toHuman()
    {
        return $this->map(function ($item) {
            ...
            $item['dias_vencimiento_unidad'] = $item['dias_vencimiento_unidad'] == 1 ? 'Días' : 'Meses';
            $item['dias_vencimiento_tipo'] = $item['dias_vencimiento_tipo'] == 1 ? 'Calendario' : 'Hábiles';
            $item['dias_vencimiento'] = number_format($item['dias_vencimiento'], 0, ',', '.');
            // $item['valor_actuacion'] = '$ ' . number_format($item['valor_actuacion'], 0, ',', '.');
            if ($item['estado_actuacion']) {
